...
class BaseTrainer(object):
    ...
    # -------------------
    def get_epoch_number(self, filename):
        ...
        if not saveall:
            for model in self.models:
                if model.filepath in save_fps:
                    shutil.copy(model.filepath, f"{os.path.dirname(os.path.realpath(model.filepath))}/model-best.epoch_0")
                    continue
    ...
    def run(self, start_epoch, decode_fn=None):
        ...
        for epoch_idx in range(start_epoch, max_epochs):
            ...
            if not (epoch_idx and (epoch_idx % eval_every == 0
                                   or epoch_idx + 1 == max_epochs)) or epoch_idx < (max_epochs / 3):
                continue
            with torch.no_grad():
                devloss = self.calc_loss(DEV, params.bs, epoch_idx)
                # -------------
                # eval_res = self.evaluate(DEV, epoch_idx, decode_fn)
                eval_res = self.fast_evaluate(DEV, epoch_idx, params.bs)
                # -------------
