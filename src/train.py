...
class Trainer(BaseTrainer):
    ...
    def fast_evaluate(self, mode, epoch_idx, batch_size):
        self.model.eval()
        ...
        for batch in tqdm(sampler(batch_size), total=nb_batch):
            b_s = batch[0].shape[1] # get second dimensions from source
            nb_sample += b_s
            correct += model.get_correct_preds(batch)
        acc = round(float(correct) / nb_sample * 100, 4)
        distance = 1
        results = [util.Eval('acc', 'accuracy', acc), util.Eval('dist', 'average edit distance', distance)]
        for result in results:
            self.logger.info(
                f'{mode} {result.long_desc} is {result.res} at epoch {epoch_idx}'
            )
