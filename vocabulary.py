...
def write_vocab_to_file(vocab_list, vocab_file_path):
    """ Writes given vocabulary object to file """
    vocab_file = open(vocab_file_path, "w", encoding='utf-8')  # "ISO-8859-1")
    # Get counter object to hold counts of characters
    vocab_counter = collections.Counter(vocab_list)

    # Write vocabulary to file in order of frequency
...
def create_vocab_files(src_file_path, vocab_file_path):
    """Reads conll file and creates input tokens vocabulary and target tokens vocabulary, and w
	rites them in different files """
    # Read file, and splits to input tokens and target tokens.	Each input and target is a list of tokens
    ...
    # input_vocab_list = lemmas_tokens + targets_tokens
    input_vocab_list = lemmas_tokens + features_tokens
    # write vocabularies of inputs and outputs to files
    write_vocab_to_file(input_vocab_list, vocab_file_path + "-input")
