...
def process_generated_file_to_output_file(data_dir, checkpoint_dir, language, dataset_type, dataset_mode):
    ...
    for i in [6]:
        if os.path.exists(f"{checkpoint_dir}/{dataset_type}/{i}"):
            ...
            if not os.path.exists(f"results/{dataset_mode}/{i}/"):
                os.makedirs(f"results/{dataset_mode}/{i}")
            print(f" Copied to results/{dataset_mode}/{i}/{language}.hyp")
            shutil.copy(output_file_path, f"results/{dataset_mode}/{i}/{language}.hyp")
...
# def rename_best_checkpoint(checkpoint_dir, dataset_type):
#     for i in range(1, 3):
#         if os.path.exists(f"{checkpoint_dir}/{dataset_type}/{i}/model_best"):
#             print(f"Moved file {checkpoint_dir}/{dataset_type}/{i}/model_best to "
#                   f"{checkpoint_dir}/{dataset_type}/{i}/model_best.epoch_0")
#             os.rename(f"{checkpoint_dir}/{dataset_type}/{i}/model_best",
#                       f"{checkpoint_dir}/{dataset_type}/{i}/model_best.epoch_0")

# rename_best_checkpoint(f"{args.checkpoint_dir}/{lang}", "aug")
